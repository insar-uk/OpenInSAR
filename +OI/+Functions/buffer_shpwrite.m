function buffer_shpwrite(varargin)
    ...
    for k = 1:length(S)
        x = S(k).(xField);
        y = S(k).(yField);
        [fileLengthInWords, boundingBox, index(:,k),todo] = ...
            shpWriteRecord(fid, writefcn, shapeType, x, y, k, fileLengthInWords, boundingBox,todo);
    end
    
    %WRITE EVERYTHING AT ONCE
    fwrite(fid,todo.','int32','ieee-le');
    ...
    function [fileLengthInWords, boundingBox, index,todo] = shpWriteRecord(fid,...
          writefcn, shapeType, x, y, recordNumber, fileLengthInWords, boundingBox,todo)
    % Write an SHP record for each structure element s.  Return index entry.
    
    recordHeaderLengthInWords = 4;
    offsetInWords = 50+(numel(todo)* 2);
    [contentLengthInWords, recordBoundingBox,todo] ...
        = writefcn(fid, recordNumber, shapeType, x, y,todo);
    ...
    function [contentLengthInWords, boundingBox,todo] ...
        = shpWritePoint(fid,recordNumber,shapeType,X,Y,todo)
    ...
    % todo=[todo swapbytes(int32([recordNumber contentLengthInWords]))];
    swapBytes32 = @(x) fliplr(typecast(fliplr(typecast(x,'uint8')),'int32'));
    todo=[todo swapBytes32(int32([recordNumber contentLengthInWords]))];
    % sum(uint32(bitset(0,1:32,bitget(uint32(X), 32:-1:1))))
    
    % Write the shape type.
    % fwrite(fid, int32(shapeType), 'int32', 'ieee-le');
    todo=[todo int32(shapeType)];
    
    % Write the point coordinates.
    % fwrite(fid, [X Y], 'double', 'ieee-le');
    todo=[todo typecast(double([X Y]),'int32')];
    
    
    %--------------------------------------------------------------------------
    
    function [contentLengthInWords, boundingBox,todo] ...
        = shpWriteMultiPoint(fid,recordNumber,shapeType,X,Y,todo)
    ...
    todo=[todo typecast(double(boundingBox),'int32')];
    
    % Write the number of points.
    % fwrite(fid, size(points,2), 'int32', 'ieee-le');
    todo=[todo int32(size(points,2))];
    
    % Write the point coordinates.
    % fwrite(fid, points, 'double', 'ieee-le');
    todo=[todo typecast(double(points),'int32').'];
    ...
    function todo=dbfWriteTableRecord(fid, s, fmt, nanmask,todo)
